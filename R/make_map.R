...
#get mapping baselayers ----------
    ...
    Canada <- ne_states(country = "Canada",returnclass = "sf")%>%
      ...
      st_as_sf()%>%
      st_coordinates()%>% # now create a dataframe for ggplot
      data.frame()%>%
      select(X,Y,L2)%>%
      rename(long=X,lat=Y,group=L2)
...
#Make the plot ----------
  p1 <- ggplot() +
    geom_tile(data=ras,aes(x=Longitude,y=Latitude,fill=MAP))+
    geom_polygon(data = Canada, aes(x=long, y = lat, group = group), 
                  fill = "white", color="black") + 
      geom_polygon(data = USA, aes(x=long, y = lat, group = group), 
                   fill = "white", color="black") + 
     coord_fixed(xlim = Long.lim,  ylim = Lat.lim, ratio = 1.2,expand=FALSE)+
        theme_bw()+
  ...
  #apply colour scales to plot
    ...
    if(!is.null(trans)){
        p1 <- p1+scale_fill_gradientn(name=variable,colours=colorRampPalette(rev(c("red","yellow","springgreen","royalblue")))(50),trans="log10",
